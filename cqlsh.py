...
#
try:
    # registration is otherwise lazy in Python3
...
class Shell(cmd.Cmd):
        ...
        if tablename not in ksmeta.tables:
            if ksname == 'system_auth' and tablename in ['roles', 'role_permissions']:
                ...
                raise ColumnFamilyNotFound(
                    "Column family {} not found".format(tablename))
        else:
    ...
    def get_view_meta(self, ksname, viewname):
        ...
        if viewname not in ksmeta.views:
            raise MaterializedViewNotFound(
                "Materialized view '{}' not found".format(viewname))
    ...
    def get_object_meta(self, ks, name):
        ...
        raise ObjectNotFound(
            "'{}' not found in keyspace '{}'".format(name, ks))
    ...
    def reset_prompt(self):
        if self.current_keyspace is None:
            ...
            self.set_prompt(self.keyspace_prompt.format(
                self.current_keyspace), True)
    ...
    def strip_comment_blocks(self, statementtext):
        comment_block_in_literal_string = re.search(
            '["].*[/][*].*[*][/].*["]', statementtext)
    ...
    def perform_simple_statement(self, statement):
        ...
        try:
            ...
            err_msg = ensure_text(err.message if hasattr(
                err, 'message') else str(err))
            ...
            while True:
                ...
                if result.current_rows or isFirst:
                    ...
                    self.print_static_result(
                        result, table_meta, with_header, tty, num_rows)
    ...
    def print_static_result(self, result, table_meta, with_header, tty, row_count_offset=0):
        ...
        formatted_values = [list(map(self.myformat_value, [
                                 row[c] for c in column_names], cql_types)) for row in result.current_rows]
    ...
    def do_describe(self, parsed):
        ...
        ), consistency_level=cassandra.ConsistencyLevel.LOCAL_ONE, fetch_size=self.page_size if self.use_paging else None)
        ...
        for row in rows:
            ...
            if 'range_ownership' in row:
                ...
                for entry in list(row['range_ownership'].items()):
                    print(' %39s  [%s]' % (entry[0], ', '.join(
                        [host for host in entry[1]])))
                print('')
...
def read_options(cmdlineargs, environment):
    configs = configparser.SafeConfigParser() if sys.version_info < (
